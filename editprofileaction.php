...
require('actions/users/getinfosofeditprofileaction.php');
...
if(isset($_POST['modification'])){
    ...
    if(!empty($_POST['first_name']) AND !empty($_POST['last_name']) ){
        ...
        $edituser = $bdd->prepare('UPDATE  users SET nom = ?, prenom = ? ,email=? ,description=?, compte_linkedlen = ?,num_tel=? ,date_naissance=? WHERE id = ?');
        $edituser->execute(array($new_nome, $newprenom,$newemail,$description,$cmpte_lenkdlen,$numtele,$date_naissance,$idOfUser));
        if(!empty($edituser)){
            $_SESSION['auth'] = true;
